...
export class CookieOAuthStorage extends OAuthStorage{
    ...
    getUser():CurrentUser{
        let userName = this.getItem(USERNAME_KEY);
        let profilePicUrl = this.getItem(PROFILE_PIC_KEY);
        let tok = this.getItem(ACCESSTOKEN_KEY);
        let id = this.getItem(ID_KEY);
        let user = new CurrentUser({UserName:userName,PicUrl:profilePicUrl,Id:id,Token:tok});
        return user;
    }
