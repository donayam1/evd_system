    ...
    public class ManageRoleTypeValidator : IManageRoleTypeValidator
    {
        ...
        public bool Validate(RoleTypeModel roleTypes)
        {
            if (roleTypes.ObjectStatus == ObjectStatusEnum.REMOVED) {
                if (roleTypes.Level == RoleTypeConstants.SystemAdminLevel || roleTypes.Level == RoleTypeConstants.RetailerLevel) {
            ...
            {
                if (roleTypes.Level == RoleTypeConstants.SystemAdminLevel || roleTypes.Level == RoleTypeConstants.RetailerLevel ||
                    roleTypes.Level < RoleTypeConstants.SystemAdminLevel || roleTypes.Level > RoleTypeConstants.RetailerLevel)
                {
